...
const ChatbotSettingsPage = ({ botId, onSaved, onCancel }) => {
    ...
    const [currentBotId, setCurrentBotId] = useState(botId || null);
    ...
    useEffect(() => {
        async function loadBot() {
            ...
            try {
                ...
                if (botId) {
                    const res = await fetch(`${API_BASE_URL}/api/bots/${botId}`, {
                    ...
                    if (!res.ok) {
                        ...
                        throw new Error("Failed to load your chatbot");
                    }
                    bot = await res.json();
                } else {
                    ...
                    bot = Array.isArray(bots) && bots.length > 0 ? bots[0] : null;
                }
    ...
    const handleSave = async () => {
        ...
        try {
            const hasExistingBot = Boolean(currentBotId && String(currentBotId).trim());
            const res = await fetch(
                hasExistingBot ? `${API_BASE_URL}/api/bots/${currentBotId}` : `${API_BASE_URL}/api/bots`,
                {
                    method: hasExistingBot ? "PUT" : "POST",
                    headers: {
            ...
            if (bot?.id) setCurrentBotId(bot.id);
